...
import { getPharmacyStats, getMedicines, getPrescriptions, getStockHistory } from "@/lib/pharmacy";
...
import {
  Pill, Package, AlertTriangle, TrendingDown, Search, Plus, Pencil, ArrowUpDown,
  ClipboardList, Loader2, CalendarClock, IndianRupee
} from "lucide-react";
...
const statusColors = {
  active: 'warning', partially_dispensed: 'secondary', fully_dispensed: 'success', cancelled: 'destructive'
};
...
export default function PharmacyDashboard() {
  ...
  const { data: medsRes, isLoading: medsLoading } = useQuery({
    ...
    queryFn: () => getMedicines({ search: search || undefined, category: catFilter === 'all' ? undefined : catFilter, sort: sortField }),
  });
  ...
  const toggleSort = (field) => setSortField(prev => prev === field ? `-${field}` : prev === `-${field}` ? field : field);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Medicine inventory, prescriptions & dispensing</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setRxOpen(true)}><ClipboardList className="mr-2 h-4 w-4" />New Prescription</Button>
          <Button onClick={() => { setEditMed(null); setAddOpen(true); }}><Plus className="mr-2 h-4 w-4" />Add Medicine</Button>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent><div className="text-2xl font-bold">{s.expiringIn30 || 0}</div><p className="text-xs text-muted-foreground">Within 30 days</p></CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent><div className="text-2xl font-bold">₹{((s.inventoryValue || 0) / 1000).toFixed(1)}K</div></CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="inventory" className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search name, composition…" value={search} onChange={e => setSearch(e.target.value)} className="pl-10" />
            </div>
                      ...
                      <TableRow key={med._id}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end gap-1">
                            <Button variant="ghost" size="icon" onClick={() => { setEditMed(med); setAddOpen(true); }}><Pencil className="h-4 w-4" /></Button>
                            <Button variant="ghost" size="icon" onClick={() => setStockMed(med)}><Package className="h-4 w-4" /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
                ...
                <Table>
                  ...
                  <TableBody>
                    {prescriptions.map(rx => {
                      ...
                      const dName = rx.doctor ? `Dr. ${rx.doctor.firstName || ''} ${rx.doctor.lastName || ''}`.trim() : 'Unknown';
                      return (
                        <TableRow key={rx._id}>
                          ...
                          <TableCell className="text-right">
                            {(rx.status === 'active' || rx.status === 'partially_dispensed') && (
                ...
                <TableBody>
                  {stockHistory.map(h => (
                    <TableRow key={h._id}>
                      ...
                      <TableCell>{h.previousStock} → {h.newStock}</TableCell>
