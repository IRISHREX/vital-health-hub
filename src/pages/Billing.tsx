...
import { mockInvoices, PaymentStatus } from "@/lib/mock-data";
...
export default function Billing() {
  ...
  const stats = {
    ...
    collected: mockInvoices.reduce((sum, inv) => sum + inv.paidAmount, 0),
    pending: mockInvoices
      .filter((i) => i.status !== "paid")
      .reduce((sum, inv) => sum + (inv.totalAmount - inv.paidAmount), 0),
    paidCount: mockInvoices.filter((i) => i.status === "paid").length,
    pendingCount: mockInvoices.filter((i) => i.status !== "paid").length,
  };
              ...
              {filteredInvoices.map((invoice) => {
                ...
                return (
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell
                      className={balance > 0 ? "text-status-occupied" : ""}
                    >
